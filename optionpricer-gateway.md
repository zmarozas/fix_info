...
**In-flight correlation — open detail.** When a RiskProfile update arrives on a ProductId, the bridge must know where to route the resulting snapshot. Either the topic structure carries enough to publish statelessly, or a transient in-memory ProductId -> request-context map is kept for the life of the subscription. This is transient correlation state, not persistent reference data, but it should be confirmed against the "as dumb as possible" directive.
...
4. In-flight correlation — stateless routing via topic structure, or a transient correlation map? (see section 9)
